					...
					if (!cObj->Def->NoHorizontalMove)
						if (Abs(ty - cObj->y) <= level)
							if (Abs(tx - cObj->x) <= level)
							{
								...
								auto p2 = itofix(-level + Abs(ty - cObj->y)) / BoundBy<int32_t>(cObj->Mass / 10, 4, (cObj->Category & C4D_Living) ? 8 : 20);
								auto p1 = itofix(Sign(cObj->x - tx + Rnd3()) * (level - Abs(tx - cObj->x))) / BoundBy<int32_t>(cObj->Mass / 10, 4, (cObj->Category & C4D_Living) ? 8 : 20);
					...
					if (Abs(tx - cObj->x) <= range)
						if (!Random(3))
							if (cObj->Action.t_attach)
								...
								{
									auto r1 = itofix(Rnd3());
									cObj->Fling(r1, Fix0, false, iCausedBy);
								}
}
...
void C4Game::CastObjects(C4ID id, C4Object *pCreator, int32_t num, int32_t level, int32_t tx, int32_t ty, int32_t iOwner, int32_t iController)
{
	...
	{
		auto r4 = itofix(Random(3) + 1);
		auto r3 = FIXED10(Random(2 * level + 1) - level);
		auto r2 = FIXED10(Random(2 * level + 1) - level);
		auto r1 = Random(360);
		CreateObject(id, pCreator, iOwner,
			tx, ty, r1, r2, r3, r4, iController);
	}
}
...
void C4Game::BlastCastObjects(C4ID id, C4Object *pCreator, int32_t num, int32_t tx, int32_t ty, int32_t iController)
{
	...
	{
		auto r4 = itofix(Random(3) + 1);
		auto r3 = FIXED10(Random(61) - 40);
		auto r2 = FIXED10(Random(61) - 30);
		auto r1 = Random(360);
